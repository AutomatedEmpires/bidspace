...
test("rejects inventory units outside biddable lifecycle states", () => {
  assert.doesNotThrow(() => assertInventoryUnitAcceptsBid({ status: "available" }));
  assert.doesNotThrow(() => assertInventoryUnitAcceptsBid({ status: "receiving_bids" }));
  assert.doesNotThrow(() => assertInventoryUnitAcceptsBid({ status: "shortlisted" }));
  assert.throws(
    () => assertInventoryUnitAcceptsBid({ status: "reserved" }),
