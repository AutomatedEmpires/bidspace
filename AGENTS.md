...
- **Canonical clone (the ONLY writable copy):** WSL `Ubuntu-24.04-Recovered` → `/home/jackson/automatedempires/ventures/bidspace`.
  Never clone this repository anywhere else on the machine. Parallel work uses controlled
  worktrees: `ae start bidspace -t <task> -a <agent> --worktree`.
- **Sessions:** acquire the single-writer lease first (`ae start bidspace -t <task> -a <agent>`);
  end with `ae finish bidspace`. Work counts as done ONLY when pushed and remote-SHA-verified.
...
- **Providers (fixed — never swap or cross-wire):** db=supabase (packages/db/migrations, latest 0014), auth=clerk.
- **Warn before:** running migrations
- Full policy: `github.com/AutomatedEmpires/ae-control` → `POLICY.md`. Briefing: `ae info bidspace`.
