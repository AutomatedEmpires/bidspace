...
for `AutomatedEmpires/bidspace`, and writes the issue URL back to the Notion
...
| --- | --- | --- |
| `GITHUB_TOKEN` | yes | Fine-grained PAT, **Issues: Read and write** on `AutomatedEmpires/bidspace`. |
...
```bash
pnpm --filter @bidspace/notion-github-issue-dispatcher check   # tsc --noEmit
ntn doctor
