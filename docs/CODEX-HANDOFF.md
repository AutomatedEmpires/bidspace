...
- **Software**: 43 routes, typecheck 5/5, 73 unit tests, green build, CI green.
...
| Command | Proves |
|---|---|
| `pnpm typecheck && pnpm lint && pnpm test && pnpm --filter @bidspace/web build` | code health (5/5, 73 tests, green) |
...
| `set -a && source apps/web/.env.local && set +a && pnpm --filter @bidspace/web exec tsx ../../tools/live-duplicate-check.ts` | recurring copy-forward |
| `set -a && source apps/web/.env.local && set +a && pnpm --filter @bidspace/web exec tsx ../../tools/verify-sentry.ts` | Sentry event delivery |
