...
- Product/vision conflict → Notion. Implementation conflict → repo. (DECISIONS D018.)
...
5. **Reconciled source-of-truth** — `README.md` + `docs/DECISIONS.md`: added **D018** (Notion=product truth, repo=implementation truth), marked D017's blanket "repo wins" partially superseded.
6. **Added CI** — `.github/workflows/ci.yml` runs typecheck + lint + build on PRs.
...
- Keep CI green; add typecheck/lint/build scripts as the monorepo grows.
