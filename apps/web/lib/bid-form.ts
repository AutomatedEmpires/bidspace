...
export function getBidAvailabilityError(
  ...
  inventoryUnitStatus?: InventoryUnitStatus,
): string | null {
  if (
    inventoryUnitStatus &&
    !BIDDABLE_INVENTORY_UNIT_STATUSES.includes(inventoryUnitStatus)
  ) {
    return `Inventory unit is not accepting bids (status: ${inventoryUnitStatus})`;
  }
