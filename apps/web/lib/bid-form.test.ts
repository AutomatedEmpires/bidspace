...
test("getBidAvailabilityError treats a zero minimum as an open positive-price bid", () => {
  assert.equal(getBidAvailabilityError("receiving_bids", 0, "receiving_bids"), null);
});

test("getBidAvailabilityError rejects inventory units outside biddable states", () => {
  assert.match(
    getBidAvailabilityError("receiving_bids", 1_000, "reserved") ?? "",
    /inventory unit is not accepting bids/i,
  );
  assert.equal(getBidAvailabilityError("receiving_bids", 1_000, "available"), null);
  assert.equal(getBidAvailabilityError("receiving_bids", 1_000, "shortlisted"), null);
});
