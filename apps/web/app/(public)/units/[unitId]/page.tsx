...
export default async function UnitDetailPage({
  ...
  async function submitBidAction(_state: BidFormState, formData: FormData): Promise<BidFormState> {
    ...
    try {
      ...
      const currentBidAvailabilityError = getBidAvailabilityError(
        currentOpportunity.status,
        currentUnit.minimum_bid_cents ?? currentOpportunity.minimum_bid_cents,
        currentUnit.status,
      );
      if (currentBidAvailabilityError) {
        return { status: "error", message: currentBidAvailabilityError };
      }
